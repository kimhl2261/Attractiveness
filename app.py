...
def load_spot_csv(csv_url: str) -> pd.DataFrame:
    ...
    for col in [
        "category", "district", "address", "operation_hours", "free_type", "fee",
...
def parse_population(raw: dict) -> dict:
    ...
    if result is not None and result.get("CODE") != "INFO-000":
        return empty
...
def load_all_data(csv_url: str, api_key: str | None) -> pd.DataFrame:
    ...
    if api_key:
        ...
        df = df.merge(
            live[["spot_name", "congestion", "congestion_message", "male_rate", "female_rate", "ppltn_min", "ppltn_max"]],
            on="spot_name",
            how="left"
        )
    else:
...
def fetch_image(homepage_url: str, spot_name: str) -> str | None:
        ...
        try:
            ...
            for img in soup.find_all("img", src=True):
                ...
                try:
                    ...
                    w, h = 200, 150
                if w >= 100 and h >= 80 and _valid_img(src):
                    return src
        except Exception:
            pass
    ...
    try:
        r = requests.get(
            f"https://search.naver.com/search.naver?where=image&query={quote(spot_name + ' 서울 야경')}",
            headers=_HDR,
            timeout=7,
        )
        soup = BeautifulSoup(r.text, "html.parser")
        for img in soup.select("img._image,.image_result img"):
            ...
            if src and src.startswith("http") and _valid_img(src):
                return src
    except Exception:
        pass
...
if page == "🏠 홈":
    ...
    if not api_key:
        st.warning(
            ...
            "> 샘플 키는 광화문·덕수궁 단 1곳만 조회 가능할 수 있습니다."
        )

    st.markdown("### 지금 가기 좋은 명소")
    show_img = st.toggle("대표 이미지 표시", value=False, help="홈페이지에서 이미지를 불러옵니다")
...
# ── 탐색 ─────────────────────────────────────────────────────────
    ...
    with f1:
        keyword = st.text_input("🔎 검색", "", placeholder="명소 이름 검색...")
    with f2:
        sel_cat = st.selectbox("분류", ["전체"] + sorted([x for x in df["category"].dropna().unique() if x]))
    with f3:
        sel_cong = st.selectbox("혼잡도", ["전체", "여유", "보통", "붐빔"])
    with f4:
        sel_dist = st.selectbox("지역구", ["전체"] + sorted([x for x in df["district"].dropna().unique() if x]))
    with f5:
        sel_park = st.selectbox("주차", ["전체", "가능", "불가"])
    ...
    if sel_cong != "전체":
        fdf = fdf[fdf["congestion"] == sel_cong]
    if sel_dist != "전체":
        fdf = fdf[fdf["district"] == sel_dist]
    if sel_park != "전체":
        fdf = fdf[fdf["parking_available"] == sel_park]
    ...
    with tab_card:
        ...
        if len(fdf) == 0:
            ...
            for i in range(0, len(pairs), 2):
                c1, c2 = st.columns(2)
                with c1:
                    render_card(pairs[i][1], show_image=show_img2)
                if i + 1 < len(pairs):
                    with c2:
                        render_card(pairs[i + 1][1], show_image=show_img2)

    with tab_map:
        if len(fdf) == 0:
            st.info("표시할 명소 없음")
        else:
            st_folium(make_map(fdf), height=560, width=None, returned_objects=[])

    with tab_data:
        show_cols = ["spot_name", "category", "district", "operation_hours", "free_type", "parking_available", "congestion", "congestion_message"]
        st.dataframe(fdf[[c for c in show_cols if c in fdf.columns]], use_container_width=True, hide_index=True)
...
# ── 상세 ─────────────────────────────────────────────────────────
    ...
    with left:
        ...
        if msg:
            ...
            st.markdown(
                f'<div style="background:{CONGESTION_BG.get(cong,"#21262d")};border:1px solid {color}40;border-radius:8px;padding:10px 14px;color:{color};font-size:13px;margin-bottom:12px;">💬 {msg}</div>',
                unsafe_allow_html=True,
            )
    ...
    with right:
        ...
        pmin = row.get("ppltn_min")
        pmax = row.get("ppltn_max")
        mrate = row.get("male_rate")
        frate = row.get("female_rate")
        ...
        if cong:
            ...
            with c1r:
                st.metric("혼잡도", cong)
                if mrate:
                    st.metric("남성 비율", f"{mrate}%")
            with c2r:
                if pmin and pmax:
                    st.metric("추정 인구", f"{pmin}~{pmax}명")
                if frate:
                    st.metric("여성 비율", f"{frate}%")
        else:
            if api_key:
                st.markdown('<div style="color:#8b949e;font-size:13px;background:#21262d;border-radius:8px;padding:12px;">이 장소는 실시간 API 미지원 또는 응답 없음 상태입니다.</div>', unsafe_allow_html=True)
            else:
...
# ── 서비스 소개 ───────────────────────────────────────────────────
    ...
    샘플 키는 일부 장소만 조회 가능할 수 있습니다.<br>
    특히 광화문·덕수궁 외에는 대부분 <code>ERROR-500</code>이 반환될 수 있습니다.<br><br>
    <b style="color:#d29922;">✅ 해결 방법</b><br>
    1. 서울 열린데이터광장에서 정식 API 키 발급<br>
    2. Streamlit Secrets에 아래 내용 추가:
    ...
    with st.expander("🅿️ 주차 현황"):
        pv = df["parking_available"].value_counts(dropna=False).reset_index()
    ...
    with st.expander("🔬 API 직접 테스트"):
        st.markdown('<div style="color:#8b949e;font-size:13px;">장소명을 직접 입력해 원본 응답과 파싱 결과를 확인합니다.</div>', unsafe_allow_html=True)
