...
def load_spot_csv(url: str) -> pd.DataFrame:
    ...
    df["api_place_name"] = df["spot_name"].map(lambda x: SPOT_TO_API.get(x, None))
...
def fetch_image(homepage_url: str, spot_name: str) -> str | None:
        ...
        try:
            ...
            for img in soup.find_all("img", src=True):
                ...
                try:
                    ...
                    w, h = 200, 150
                if w >= 100 and h >= 80 and _valid(src):
                    return src
        except Exception:
            pass
    ...
    try:
        ...
        for img in soup.select("img._image,.image_result img"):
            ...
            if src and src.startswith("http") and _valid(src):
                return src
    except Exception:
        pass
    return None
...
# ════════════════════════════════════════════════════════════════
    ...
    with f1:
        kw = st.text_input("🔎 검색", "", placeholder="명소 이름 검색...")
    with f2:
    ...
    fdf = df.copy()
    if kw.strip():
        fdf = fdf[fdf["spot_name"].str.contains(kw.strip(), case=False, na=False)]
    if sel_cat != "전체":
...
# ════════════════════════════════════════════════════════════════
    ...
    with left:
        with st.spinner("이미지 불러오는 중..."):
            img = fetch_image(row.get("homepage_url", ""), sel)
        if img:
            st.image(img, use_container_width=True)
    ...
    with right:
        ...
        if cong:
            ...
            with c1r:
                st.metric("혼잡도", cong)
                if row.get("male_rate"):
                    st.metric("남성 비율", f'{row["male_rate"]}%')
            with c2r:
                pmin = row.get("ppltn_min")
                pmax = row.get("ppltn_max")
                if pmin and pmax:
                    st.metric("추정 인구", f"{pmin}~{pmax}명")
                if row.get("female_rate"):
                    st.metric("여성 비율", f'{row["female_rate"]}%')
        else:
            msg_text = (
                "API 미지원 장소입니다." if api_key and not api_name else
...
# ════════════════════════════════════════════════════════════════
    ...
    with st.expander("🗺️ CSV 명소 ↔ API 장소명 매핑 현황"):
        ...
        mv["매핑"] = mv["api_place_name"].apply(lambda x: "✅ 매핑됨" if pd.notna(x) and x else "❌ 미매핑")
        mv["혼잡도 수신"] = mv["congestion"].apply(lambda x: "✅ " + str(x) if pd.notna(x) else "—")
    ...
    with st.expander("🔬 API 직접 테스트"):
        ...
        ca, cb2 = st.columns(2)
        with ca:
            sel_test = st.selectbox("매핑된 장소 선택", ["직접 입력"] + mapped_places)
        with cb2:
            manual = st.text_input("또는 직접 입력", "", placeholder="광화문·덕수궁")
